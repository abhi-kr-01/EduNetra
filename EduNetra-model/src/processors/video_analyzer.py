...
class VideoAnalyzer:
    ...
    def analyze_emotions(self, frame):
        ...
        """
        try:
            # DeepFace expects an image array. 
            # enforce_detection=False prevents a crash if no face is found in the specific frame.
            result = DeepFace.analyze(
                img_path=frame, 
                actions=['emotion'], 
                enforce_detection=False,
                detector_backend='opencv'  # 'opencv' is faster; use 'retinaface' for higher accuracy
            )
            ...
            # Fallback if analysis fails (e.g., blur, extreme angle, or no face confident enough)
